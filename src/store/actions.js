...
import { FETCH_SITES_BEGIN, FETCH_SITES_SUCCESS, FETCH_SITES_FAILURE, TEXT_SEARCH_CHANGE, REGION_CLICK, SITE_SELECT,
    FETCH_SITE_DATA_BEGIN, FETCH_SITE_DATA_SUCCESS, FETCH_SITE_DATA_FAILURE, FETCH_OVERLAY_SITE_DATA_SUCCESS
} from './constants'
...
export const fetchSiteData = (site_id, isOverlay=false) => {
    ...
    return dispatch => {
        ...
        axios.get(url)
            .then(siteData => {
                dispatch({ type: isOverlay ? FETCH_OVERLAY_SITE_DATA_SUCCESS : FETCH_SITE_DATA_SUCCESS, payload: { siteData } });
            })
